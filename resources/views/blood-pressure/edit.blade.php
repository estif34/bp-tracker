        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-md rounded-lg">
                <div class="bg-gradient-to-r from-indigo-600 to-blue-500 px-6 py-4">
                    <h3 class="text-white font-medium">Edit Blood Pressure Reading</h3>
                </div>

                <form method="POST" action="{{ route('blood-pressure.update', $bloodPressureReading) }}" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mt-6">
                        ...
                        <div class="mt-1">
                            <textarea id="note" name="note" rows="3" 
                                     class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-md @error('note') border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500 @enderror">{{ old('note', $bloodPressureReading->note) }}</textarea>
                        </div>
                        @error('note')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
